...
6. `queue/` — scan all subdirs (`pending/`, `approved/`, `in-progress/`, `diffhub-review/`, `waiting/`, `done/`, `blocked/`) to map the task landscape
...
```bash
"$CRAFT_ROOT/plugins/orchestrator-skills/scripts/start-discoverer" \
    "<topic-slug>" \
    "<one-line topic>" \

...
Move the task into the new `diffhub-review` queue state and run the local babysit loop. Bot reviewer findings AND human comments appear through diffhub's comments API in parallel; the agent addresses each new actionable comment as it arrives.
...
5. Run the wake-up loop. **You MUST run this `Bash` tool call and block on it** — without it, the watcher writes to disk and nothing happens:
   ...
   >   diffhub's comments API / `.orchestrator/diffhub-pending.md`

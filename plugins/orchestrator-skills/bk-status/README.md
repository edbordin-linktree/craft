...
```bash
cd plugins/orchestrator-skills/bk-status
bun install
...
```bash
"$CRAFT_ROOT/plugins/orchestrator-skills/scripts/show-build-status" <pr-url>
```

That:
1. Ensures the bk-status server is up (spawns via launchctl if needed).
